...
export default {
  ...
  data () {
    return {
      listId: 'NetworkList',
      cloudEnvOptions: getCloudEnvOptions('network_manage_brands').filter(item => item.key !== 'onpremise'),
      cloudEnv: '',
      resStaticsResource: 'networks',
    }
  },
  methods: {
    getStatusOpts (data) {
      ...
      const statusOpts = [
        ...
        { title: this.$t('status.network.available'), type: 'available', num: available?.total_count || 0 },
