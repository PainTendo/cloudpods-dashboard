...
export default {
  ...
  async created () {
    // 自动登录
    const { user, token, timestamp, redirect } = this.$route.query
    const autoLoginSuccess = await autoLogin({ store: this.$store, router: this.$router }, user, token, timestamp, redirect)
    if (autoLoginSuccess) {
      return
    }
